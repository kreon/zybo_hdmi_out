...
#define HDMI_OUT_VTC_ID 		XPAR_VTC_0_DEVICE_ID
//#define HDMI_IN_VTC_ID 			XPAR_V_TC_IN_DEVICE_ID
#define HDMI_IN_GPIO_ID 		XPAR_AXI_GPIO_HDMI_DEVICE_ID
//#define HDMI_IN_VTC_IRPT_ID 	XPAR_FABRIC_V_TC_IN_IRQ_INTR
#define HDMI_IN_GPIO_IRPT_ID 	XPAR_FABRIC_AXI_GPIO_HDMI_IP2INTC_IRPT_INTR
...
void DemoInitialize()
{
	...
	v_tpg();//fpga test pattern generator

	/*
	 * Initialize the Video Capture device
	 */
	Status = VideoInitialize(&videoCapt, &intc, &vdma, HDMI_IN_GPIO_ID, pFrames, DEMO_STRIDE, DEMO_START_ON_DET);
	if (Status != XST_SUCCESS)
	{
		xil_printf("Video Ctrl initialization failed during demo initialization%d\r\n", Status);
		return;
	}
	videoCapt.timing.HActiveVideo = 1280;
	videoCapt.timing.VActiveVideo = 1024;
	/*
