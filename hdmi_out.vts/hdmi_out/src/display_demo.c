...
#define OSDHEIGHT (u32)(osd_size*8)
/* ------------------------------------------------------------ */
/*				Global Variables								*/
/* ------------------------------------------------------------ */
 ...
 */
u8 frameBuf[DISPLAY_NUM_FRAMES][DEMO_MAX_FRAME] __attribute__((aligned(0x20)));//0x20=32b
...
void DemoInitialize()
{
	 ...
	 */
	Status = DisplayInitialize(&dispCtrl, VDMA_ID, VTC_ID, DYNCLK_BASEADDR, pFrames, DEMO_STRIDE);
...
*/
	...
	//initial screen 640*480 DEMO_PATTERN_1
...
void DemoRun()
{
	...
	while (userInput != 'q')
	{
		...
		switch (userInput)
		{
		case '1'://1 - Change Display Resolution
			DemoChangeRes();
			break;
		case '2'://2 - Change Display Framebuffer Index
		...
		case '5'://5 - Invert Current Frame colors
			...
			break;
		case '6'://6 - Invert Current Frame colors seamlessly
			nextFrame = dispCtrl.curFrame + 1;
			if (nextFrame >= DISPLAY_NUM_FRAMES) {
				nextFrame = 0;
			}
...
void DemoChangeRes()//1 - Change Display Resolution
{
	int fResSet = 0;
	int status;
	char userInput = 0;
...
void DemoPrintTest(u8 *frame, u32 width, u32 height, u32 stride, int pattern)
{
	...
	switch (pattern)
	{
	case DEMO_PATTERN_0://Print Blended Test Pattern
	...
	case DEMO_PATTERN_1://Print Color Bar Test Pattern

		xInt = width / 7; //Seven color intervals, each with width/7 pixels
		xInc = 256.0 / ((double) xInt); //256 color intensities per interval. Notice that overflow is handled for this pattern.

		fColor = 0.0;
		wCurrentInt = 1;//current interval
		for(xcoi = 0; xcoi < (width*3); xcoi+=3)
		{
			/*
			...
			for(ycoi = 0; ycoi < height; ycoi++)
			{
				 ...
				 * by the stride instead of just 1.
				 */
				iPixelAddr += stride;//stride(step) = 1920*3RGB
			}

			fColor += xInc; //color change?
			if (fColor >= 256.0)
			{
				fColor = 0.0;
				wCurrentInt++;
	...
	case DEMO_PATTERN_2://8 - Print OSD

		for(xcoi = 0; xcoi < (width*NUM_COMPONENTS); xcoi+=3)
		{
			if(xcoi < 425*NUM_COMPONENTS){
				wRed = argus_sz[xcoi];
...
void MixOSDFrame(u8 *srcFrame, u8 *destFrame, u32 width, u32 height, u32 stride)//7 - Osd Set Title
{//pic w425*h397	data_size = 168725 * LV_COLOR_SIZE / 8,
	u32 xcoi, ycoi;
	u32 lineStart= 0;
	u32 pic_width= 425, pic_high= 397, pic_sz=506175;
	//draw pic argus_sz
	for(ycoi = 0; ycoi < height; ycoi++)
	{
...
/*Red*/		destFrame[xcoi*3 + lineStart + 2] = argus_sz[xcoi*3 + 1 + ycoi*pic_width]; //Blue
	...
	// left white border
//	destFrame[osdbufpos++] = 0xff;
//	destFrame[osdbufpos++] = 0xff;
//	destFrame[osdbufpos++] = 0xff;
//
//	for (int i = 0; i < 8; i++)
//	{
//		destFrame[osdbufpos++] = 255 ^ *srcFrame;
//		destFrame[osdbufpos++] = 255 ^ *srcFrame++;
//	}
//
//	// right white border
